...
def domain_recursion(domain: list[Const],
                     ...
                     cc_xpred: dict[Pred, int],
                     cur_model, node_index: int = None, block_status: dict[Const, list[bool]] = None):
    if domain.__len__() == 0:
        global MODEL_COUNT
        MODEL_COUNT += 1
    ...
    pair_recursion(ego_element=ego_element, domain_todo=domain, domain_done=[],
                           ...
                           cur_model=cur_model, subroot_node_index=node_index, block_status=block_status)
...
def create_node(type: str, value: int = None):
...
def pair_recursion(ego_element: Const,
                           ...
                           cur_model, subroot_node_index: int = None, block_status = None):
    
    if domain_todo.__len__() == 0:
        cc_xpred[evidence_dict[ego_element]] -= 1
        del evidence_dict[ego_element]
        domain_recursion(domain_done, evidence_dict, cc_xpred, cur_model, subroot_node_index, block_status)
        return
    ...
    if subroot_node_index == None:
        subroot_or_node = create_node('Or')
    else:
        subroot_or_node = NODES[subroot_node_index]
    for idx, rel in enumerate(Rel_Dict[(Domain_to_Cell[ego_element], Domain_to_Cell[cur_element])]):
        ...
        if Config_SAT_pre.check_config_by_cache(tuple([new_cc_xpred[key] for key in sorted(new_cc_xpred.keys(), key=lambda x: int(x.name[2:]))])):
            ...
            # update block status
            ego_element_block_status = block_status[ego_element].copy()
            cur_element_block_status = block_status[cur_element].copy()
            for atom in rel:
                if atom.pred.name.startswith('@R') and atom.positive:
                    if atom.args[0] == X:
                        block_status[ego_element][int(atom.pred.name[2:])] = True
                    else:
                        block_status[cur_element][int(atom.pred.name[2:])] = True
            # judge if there is a block
            block_flag = False
            for _,v in block_status.items():
                if not all(v):
                    block_flag = True
                    break
            if not block_flag:
                # directly construct all remaining relations
                
                and_node = create_node('And')
                subroot_or_node.children.append(and_node.index)
                and_node.children.append(f'{idx}({ego_element}, {cur_element})')
                
                tmp_domain = domain_todo.copy() + domain_done.copy()
                for i, e_1 in enumerate(tmp_domain):
                    for j, e_2 in enumerate(tmp_domain):
                        if j <= i:
                            continue
                        tmp_or_node = create_node('Or')
                        and_node.children.append(tmp_or_node.index)
                        for tmp_idx, tmp_rel in enumerate(Rel_Dict[(Domain_to_Cell[e_1], Domain_to_Cell[e_2])]):
                            tmp_or_node.children.append(f'{tmp_idx}({e_1}{e_2})')
                for e_1 in domain_todo:
                    tmp_or_node = create_node('Or')
                    and_node.children.append(tmp_or_node.index)
                    for tmp_idx, tmp_rel in enumerate(Rel_Dict[(Domain_to_Cell[ego_element], Domain_to_Cell[e_1])]):
                        tmp_or_node.children.append(f'{tmp_idx}({ego_element}{e_1})')                            
                        
                block_status[ego_element] = ego_element_block_status
                block_status[cur_element] = cur_element_block_status
                continue
            ...
            # add new node
            # if only the last relation
            if domain_done.__len__() == 1 and domain_todo.__len__() == 0:
                subroot_or_node.children.append(f'{idx}({ego_element}{cur_element})')
            else:
                and_node = create_node('And')
                subroot_or_node.children.append(and_node.index)
                and_node.children.append(f'{idx}({ego_element}{cur_element})')
                new_or_node = create_node('Or')
                and_node.children.append(new_or_node.index)
            
            subroot_node_index = None if domain_done.__len__() == 1 and domain_todo.__len__() == 0 else new_or_node.index
            
            pair_recursion(
                ego_element, domain_todo.copy(), domain_done.copy(),
                new_evidence_dict, new_cc_xpred,
                new_cur_model, subroot_node_index, block_status)
            
            # update block status again
            block_status[ego_element] = ego_element_block_status
            block_status[cur_element] = cur_element_block_status
...
if __name__ == '__main__':
            ...
            with open(f'{args.input[:-7]}/cache/{args.input[:-7]}.{DOMAIN_SIZE}.cache', 'r') as file:
                for node in file:
                    ...
                    if node:
                        ...
                        META_CONFIG_DICT[cc.index(1)].add(cc)
        else: 
            # build Config_SAT_pre.META_CONFIG_DICT
    ...
    for k, v in Config_SAT_pre.META_CONFIG_DICT.items():
        ...
        def check_symmetric_positions(tuples_set):
            # 将集合转换为列表，方便操作
            tuples_list = list(tuples_set)
            n = len(tuples_list[0])  # 获取元组的长度（即列数）

            # 遍历所有可能的列对 (i, j)
            for i in range(5, n):
                for j in range(i + 1, n):
                    # 生成交换第 i 个和第 j 个位置后的集合
                    swapped_set = set()
                    for t in tuples_list:
                        # 交换元组中第 i 个和第 j 个位置的元素
                        swapped_tuple = list(t)
                        swapped_tuple[i], swapped_tuple[j] = swapped_tuple[j], swapped_tuple[i]
                        swapped_set.add(tuple(swapped_tuple))

                    # 比较交换后的集合与原集合是否相同
                    if swapped_set == tuples_set:
                        print(f"存在对称的位置：第 {i} 个位置和第 {j} 个位置")

            return

        check_symmetric_positions(v)
                        ...
                        for config in generate_sat_configs(DOMAIN_SIZE, tpl_config, ori_delta):
                            ...
                            for domain_partition in cell_assignment(context.domain, config):
                                ...
                                for element in domain_list:
                                    block_status[element] = [False for _ in range(context._m)]
                                    ...
                                    for pred in element_cell.preds:
                                        if pred.name.startswith('@R') and element_cell.is_positive(pred):
                                            ...
                                            block_status[element][pred_idx] = True
                                    
                                
                                domain_recursion(domain_list, evidence_dict, cc_xpred.copy(), 
                                                 np.full((DOMAIN_SIZE, DOMAIN_SIZE), -1, dtype=np.int8), None, block_status)
    ...
    if MODEL_COUNT != 0:
        ...
        avg_time = -1
    res = f'{DOMAIN_SIZE}, {preprocess_time}, {enumeration_time}, {num_meta_cc}, {MODEL_COUNT}, 0, {round(avg_time, 7)}\n'
    print(res)
