...
from counting_fo2.context import WFOMCContext, WFOMSContext
...
def get_template_config(context: WFOMSContext, cell_graph: CellGraph, uni_cell_graph: CellGraph, domain_size) -> set[tuple[int, ...]]:
...
def generate_raw_config(len: int, delta: int):
    for k in range(1, len+1):
        for indices in combinations(range(len), k):
            yield tuple(delta if i in indices else 0 for i in range(len))
   
   
@functools.lru_cache(maxsize=None)
def sat_config(config: tuple):
    if config in [(1, 1, 2), (2, 0, 1), (2, 1, 0), (1,2,1)]:
        return True
    return False
